...
ros::Publisher _pub_cloud, _pub_worldcloud;
ros::Subscriber _odom_sub;
ros::Subscriber _global_map_sub;
ros::Timer _local_sensing_timer, _local_sensing_timer2;
 ...
 */
void idx2Pt(int x, int y, double dis, Eigen::Vector3d &pt)
{
...
void idx2Pt2(int x, int y, const Eigen::Vector3d &t, const Eigen::Matrix3d &R, double dis, Eigen::Vector3d &pt) {
  double vtc_rad = y * _vtc_resolution_rad - _vtc_laser_range_rad / 2.0;
  double hrz_rad = x * _hrz_resolution_rad - M_PI;
...
void pubTF()
{
  if (!_has_odom)
  {
    ROS_WARN("[Laser sim] No odometry information.");
    return;
  }
  ...
  _transformStamped.header.stamp = _odom.header.stamp;

  _transformStamped.header.frame_id = "world";
  _transformStamped.child_frame_id = "robot_"+std::to_string(car_id);
  _transformStamped.transform.translation.x = _odom.pose.pose.position.x;
  _transformStamped.transform.translation.y = _odom.pose.pose.position.y;
  _transformStamped.transform.translation.z = _odom.pose.pose.position.z;
  _transformStamped.transform.rotation.x = _odom.pose.pose.orientation.x;
  _transformStamped.transform.rotation.y = _odom.pose.pose.orientation.y;
  _transformStamped.transform.rotation.z = _odom.pose.pose.orientation.z;
  _transformStamped.transform.rotation.w = _odom.pose.pose.orientation.w;
...
void renderSensedPoints(const ros::TimerEvent &event)
{
  ...
  if (!_has_global_map)
  {
    ROS_WARN("[Laser sim] No global map received yet.");
    return;
  }
  if (!_has_odom)
  {
  ...
  _local_map_pcd.header.frame_id = "robot_"+std::to_string(car_id);
...
void renderSensedPoints_world(const ros::TimerEvent &event) {
  double this_time = ros::Time::now().toSec();
    ...
    for (int x = 0; x < _hrz_laser_line_num; ++x)
      for (int y = 0; y < _vtc_laser_line_num; ++y) {
        ...
        */
        ...
        Eigen::Vector3d p;
        if (idx_map(x, y) != -1) {
          idx2Pt2(x, y, laser_t, rot, dis_map(x, y), p);
          // p = rot.transpose() * (p - laser_t);
          pt.x = p[0];
          pt.y = p[1];
          pt.z = p[2];
          // pt = _cloud_all_map.points[_pointIdxRadiusSearch[idx_map(x, y)]];
  ...
  _pub_worldcloud.publish(_local_map_pcd);
  // ROS_INFO("all cost %lf ms.", (ros::Time::now().toSec() - this_time) * 1000.0f);
}
...
int main(int argc, char **argv)
{
  ...
  _pub_worldcloud = nh.advertise<sensor_msgs::PointCloud2>(laser_pcd_topic + "_world", 10);
  ...
  _local_sensing_timer2 = nh.createTimer(ros::Duration(sensing_duration), renderSensedPoints_world);
