      ...
      odom_not_subscribe_count_(0), local_map_not_subscribe_count_(0), scan_not_subscribe_count_(0)
{
    ...
    p1.x = 0.7;
    p1.y = 0.5;
    footprint_.polygon.points.push_back(p1);
    p1.x = 0.7;
    p1.y = -0.5;
    footprint_.polygon.points.push_back(p1);
    p1.x = -0.7;
...
DWAPlanner::dwa_planning(const Eigen::Vector3d &goal, std::vector<std::pair<std::vector<State>, bool>> &trajectories)
{
    ...
    std::vector<Cost> costs;                                    // costs和trajectories的size相同，一一对应
    ...
    for (int i = 0; i < velocity_samples_; i++)
    {
        ...
        {
            std::pair<std::vector<State>, bool> traj;
            traj.first = generate_trajectory(v, 0.0);           // 匀速直线运动
            Cost cost = evaluate_trajectory(traj.first, goal);
            costs.push_back(cost);
            if (cost.obs_cost_ == 1e6)
            {
    ...
    if (available_traj_count == 0)              // 前面都无解的情况，生成静止的运动
    {
...
geometry_msgs::Twist DWAPlanner::calc_cmd_vel(void)
{
    ...
    try
    {
        // tf变换，把目标点的坐标变换到机器人base_link坐标系下
        listener_.transformPose(robot_frame_, ros::Time(0), goal_, goal_.header.frame_id, goal_base);
    }
    catch (tf::TransformException ex)
    {
        ROS_ERROR("%s", ex.what());
    ...
    // 截取目标点
    Eigen::Vector2d goal_2d(goal_base.pose.position.x, goal_base.pose.position.y);
    Eigen::Vector2d goal_tmp;

    if(goal_2d.norm() > 5.0) {
        goal_tmp = goal_2d / goal_2d.norm() * 5.0;
    } else {
        goal_tmp = goal_2d;
    }

    std::cout << "goal_2d  := " << goal_2d.transpose() << std::endl;
    std::cout << "goal_tmp := " << goal_tmp.transpose() << std::endl;
    std::cout << "--------------------------------------------" << std::endl;
    
    const Eigen::Vector3d goal(goal_tmp[0], goal_tmp[1], tf::getYaw(goal_base.pose.orientation));

    // const Eigen::Vector3d goal(goal_base.pose.position.x, goal_base.pose.position.y, tf::getYaw(goal_base.pose.orientation));

    if (dist_to_goal_th_ < goal.segment(0, 2).norm() && !has_reached_)       // 根据x和y的位置误差，判断是否到达终点附近
    {
        if (can_adjust_robot_direction(goal))               // 处理终点在机器人后方的情况，先原地旋转调转方向
        {
            // 原地旋转策略，选择最近的一个方向
    ...
    visualize_trajectory(best_traj.first, 1.0, 0.0, 0.0, selected_trajectory_pub_);
    visualize_trajectories(trajectories, 0.0, 0.0, 1.0, candidate_trajectories_pub_);
    if (use_footprint_)
...
bool DWAPlanner::can_adjust_robot_direction(const Eigen::Vector3d &goal)
{
    ...
    if (!check_collision(traj))     // 挺奇怪的，原地旋转也要判断碰撞？
        return true;
    else
        return false;
}
...
void DWAPlanner::raycast_worldObs(const nav_msgs::OccupancyGrid &map)
{
    obs_list_.poses.clear();

    // hypot表示计算斜边，即求sqrt(x^2 + y^2)
    const double max_search_dist = hypot(map.info.origin.position.x, map.info.origin.position.y);

    int odom_x = odom_now_.pose.pose.position.x;
    int odom_y = odom_now_.pose.pose.position.y;
    ...
    for (float angle = -M_PI; angle <= M_PI; angle += angle_resolution_)
    {
        for (float dist = 0.0; dist <= max_search_dist; dist += map.info.resolution)
        {
            ...
            // 索引到world系下的地图
            const int index_x = floor((pose.position.x + odom_x - map.info.origin.position.x) / map.info.resolution);
            const int index_y = floor((pose.position.y + odom_y - map.info.origin.position.y) / map.info.resolution);

            if ((0 <= index_x && index_x < map.info.width) && (0 <= index_y && index_y < map.info.height))
            {
                if (map.data[index_x + index_y * map.info.width] > 0)
                {
                    obs_list_.poses.push_back(pose);
                    break;
                }
